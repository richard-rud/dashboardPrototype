d3.sankey = function() {
  var sankey = {},
      ...
      links = [],
	  // cycle features
	  cycleLaneNarrowWidth = 2,
	  cycleLaneDistFromFwdPaths = -50,  // the distance above the paths to start showing 'cycle lanes'
	  cycleDistFromNode = 0,      // linear path distance before arcing from node
    ...
    cycleLaneDistFromFwdPaths = -200,  // the distance above the paths to start showing 'cycle lanes'
    cycleDistFromNode = 60,      // linear path distance before arcing from node
    cycleControlPointDist = 30,  // controls the significance of the cycle's arc
    cycleSmallWidthBuffer = 20  // distance between 'cycle lanes'
    ;
    */
  ...
  sankey.link = function() {
    ...
    if( d.causesCycle ) {
      ...
      */
      ...
      t_y = d.target.y,
		...
		nw_x = t_x - cycleDistFromNode - d.dy,
		nw_y = ne_y,
		sw_x = nw_x,
		sw_y = t_y + d.ty + d.dy;
    ...
    + "H" + t_x 
		...
		+ "V" + ( t_y + d.ty )
		+ "H" + (sw_x - smallWidth/2)
		+ "L" + (sw_x - (cycleControlPointDist/2) + smallWidth) + "," + t_y + " " + (nw_x - (cycleControlPointDist/2) + smallWidth) + "," + (nw_y + smallWidth) + " " + nw_x + "," + (nw_y + smallWidth)
		+ "H" + (ne_x - smallWidth)
		+ "L" + (ne_x + (cycleControlPointDist/2) - smallWidth) + "," + (ne_y + smallWidth) + " " +
